 ...
 */
const FALLBACK_SIBLING_FEATURES = [
...
function loadSiblingFeatures() {
  try {
    const featuresDir = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'features', 'src');
    return readdirSync(featuresDir, { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .map((entry) => entry.name)
      .sort();
  } catch {
    return FALLBACK_SIBLING_FEATURES;
  }
}

const SIBLING_FEATURES = loadSiblingFeatures();
const SIBLING_RE = `^(\\.\\./)+(${SIBLING_FEATURES.map(escapeRegExp).join('|')})/(domain|application|infra|http)(/|$)`;
...
const prismaOnlyInInfra = [
  {
    group: ['@mitama/data', '@mitama/data/**', '@prisma/client', '@prisma/client/**'],
